...
export function DeltaCard({ delta, criteria, onDismiss }: { delta: Delta; criteria: Criterion[]; onDismiss: () => void }) {
  ...
  return (
    <div className="mx-4 mb-1 mt-2 rounded-panel bg-active/[.08] p-3 ring-1 ring-active/30 animate-rise">
      <div className="mb-1.5 flex items-center justify-between">
        <h3 className="text-[11px] font-semibold uppercase tracking-[.14em] text-active">What changed</h3>
      ...
      {shown.length > 0 && (
        <ul className={`${delta.criteria.length ? "mt-2 border-t border-active/15 pt-2" : ""} grid grid-cols-3 gap-1.5`}>
          {shown.map((q) => (
...
export function ScenarioCard({ scenario, onApply, onDismiss }: { scenario: ScenarioResult; onApply: () => void; onDismiss: () => void }) {
  ...
  return (
    <div className="mx-4 mb-1 mt-2 rounded-panel bg-info/[.07] p-3 ring-1 ring-info/30 animate-rise">
      <div className="mb-1.5 flex items-center justify-between">
        <h3 className="text-[11px] font-semibold uppercase tracking-[.14em] text-info">Scenario tested by the agent</h3>
      ...
      {scenario.quantityDeltas.filter((q) => q.key !== "averageFacilityMW" && q.key !== "annualMWh").length > 0 && (
        <ul className="mt-2 grid grid-cols-3 gap-1.5 border-t border-info/15 pt-2">
      ...
      <div className="mt-2.5 flex gap-2">
        <button onClick={onApply} className="rounded-control bg-info/20 px-3 py-1.5 text-[11px] font-semibold text-info ring-1 ring-info/30 transition hover:bg-info/30">
          Apply to brief
        </button>
